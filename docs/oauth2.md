...
User Logout (Token Revocation - Optional but Recommended for Security) - /auth/logout/ or /auth/token/revoke/ or /auth/jwt/revoke/ (POST)
...
Refresh Token Endpoint (Token Renewal) - /auth/token/refresh/ or /auth/jwt/refresh/ (POST)
...
Password Reset Request (Initiate Password Reset) - /auth/password/reset/ or /auth/password/reset/request/ (POST)
...
Password Reset Confirmation (Complete Password Reset) - /auth/password/reset/confirm/ or /auth/password/reset/verify/ (POST)
...
Password Change (Authenticated User) - /auth/password/change/ or /users/me/password/ (POST/PATCH)
...
Get User Profile Information - /users/me/ or /users/profile/ (GET)
